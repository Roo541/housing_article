...
regime analysis, and weighted ensembles taking into account performance in regimes.
This project is inspired by the phrase "wisdom of the crowd", which essentially 
says that a collective groups aggregate opinions, predictions, judgements etc. 
...
we go with would likely be weighted in favor of the friend who comes closest to
overlapping of topic of the question and that friends field of expertise. 
...
is not too far from what is actually practiced in the financial industry. We will be using Federal Funds Rate,
Housing Starts, Unemployment Rate, and CPI all matching the RedFin frequency of monthly. We take into
account look ahead bias by lagging the macro data by one month. As all but the Federal Funds Rate data
comes out near the middle of the month for the previous month one month lag is suffice to keep things
...
hence the article will focus on Linear, Lasso, Ridge, and ElasticNet regression models.
...
The second split is for the instances within dates 8/23 to 7/24. This is the full testing 
...
We then use Lasso, Ridge, and ElasticNet Regression which address, feature selection, 
...
a combination (L1 + L2) ElasticNet regression. 

The penalties terms added
...
ElasticNet is a mix of both Lasso and Ridge. It adds both penalty terms from 
...
and ElasticNet as a mix of both, with all three utilizing regularization techniques
...
essentially the same. The best performing model turned out to be Lasso, with an
...
on the macro economic dataset. We identify regimes on the same training 
...
weighted model during the testing period (8/23 to 7/24) as the regression
...
3. With the time frames being alligned with the RedFin dataset we cross-reference
...
sought after performance metric. We see that Lasso in general performed well in most
regimes. 
```{python, echo=FALSE}
...
model plots shown below. Keep in mind the reason the blue curve is shorter than the others is because
...
identified as being regime 1 through our K-Means model. And historically during
regime 1, the best performing models were Lasso and ElasticNet which
got higher weighting (a bit more than 70%) compared to Linear and Ridge Regressions (less than 30%). 
